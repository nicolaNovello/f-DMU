...
def retrieve_target_prompts(type, target, eval_json):
    ...
    target_prompts = [meta_data['target'], meta_data['anchor']] + meta_data['hard_negative']
    ...
    for example in tqdm(
        ...
        with open(f'{outpath}/caption.txt', 'a') as f1, open(f'{outpath}/images.txt', 'a') as f2:
            images = pipeline(example["prompt"], num_inference_steps=ddim_steps,
                              guidance_scale=7.5, eta=1., generator=generator).images
...
def read_csv_prompts(file_path: str, prompt_column_index: int = 0):
    prompts = []
    try:
        # Use 'newline=""' to correctly handle line endings across different OS
        with open(file_path, 'r', newline='', encoding='utf-8') as csvfile:
            reader = csv.reader(csvfile)
            for row in reader:
                if row and len(row) > prompt_column_index:
                    prompt = row[prompt_column_index].strip()
                    if prompt:
                        prompts.append(prompt)
        return prompts
    except FileNotFoundError:
        print(f"Error: CSV prompt file not found at {file_path}. Returning empty list.")
        return []
    
def getmetrics(accelerator, type, target, base_sample_root, ckpt, n_samples,
               sample_root, eval_json, numgen, base_ckpt="CompVis/stable-diffusion-v1-4", ddim_steps=50, dpm_solver=False, ring_a_bell=False):
    # load data
    print("-",type, "-" ,target)
    print("eval_json: ", eval_json)
    meta_data = json.load(open(eval_json, 'r'))[type][target]
    ...
    for cur_target, cur_numgen in zip(target_prompts, numgens):
        # load data
        if type == 'style':
            data = load_data(cur_target.replace(' ', '_').replace('-', '_'), cur_numgen)
        elif type == 'object':
            data = load_data(meta_data['anchor'].replace(' ', '_').replace('-', '_'), cur_numgen,
                             meta_data['anchor'], cur_target)
        elif type == 'nudity':
            data_tmp = load_data(meta_data['anchor'].replace(' ', '_').replace('-', '_').replace(',',''), cur_numgen,
                             meta_data['anchor'], cur_target)
            data = []
            for sentence in data_tmp:
                #print("sentence a: ", sentence)
                if "nudity, nsfw" in sentence:
                    #print("SENTENCE")
                    sentence = sentence.replace("nudity, nsfw", "person") + ", nudity, nsfw"
        ...
        if ring_a_bell:
            data = read_csv_prompts("Ring-A-Bell-main/data/InvPrompt/VanGogh_3_length_16.csv", prompt_column_index=0)
        # model generation
        ...
        if type == 'style':
            data = load_data(cur_target.replace(' ', '_').replace('-', '_'), numgen)
        elif type == 'object':
            data = load_data(meta_data['anchor'].replace(' ', '_').replace('-', '_'), numgen,
                             meta_data['anchor'], cur_target)
        elif type == 'nudity':
            data_tmp = load_data(meta_data['anchor'].replace(' ', '_').replace('-', '_').replace(',',''), numgen,
                             meta_data['anchor'], cur_target)
            data = []
            for sentence in data_tmp:
                if "nudity, nsfw" in sentence:
        ...
        # base generation
        base_sample_path = Path(base_sample_root) / ("base-" + cur_target.replace(' ', '_').replace('-', '_'))
        if accelerator.is_main_process:
            base_sample_path = safe_dir(base_sample_path)
        print("base_sample_path: ", base_sample_path)
...
def analyze_metrics(metrics_data: dict,root):

    data = metrics_data[list(metrics_data.keys())[0]]

    other_artists_accuracies = []
    other_artists_baseline_accuracies = []
    other_artists_scores = []
    other_artists_baseline_scores = []

    vangogh_accuracy = None
    vangogh_baseline_accuracy = None
    vangogh_score = None
    vangogh_baseline_score = None
    print(metrics_data)
    for key, value in data.items():

        is_vangogh_key = 'van gogh' in key

        if key.startswith('Baseline CLIP accuracy'):
            if is_vangogh_key:
                vangogh_baseline_accuracy = value
            else:
                other_artists_baseline_accuracies.append(value)
        elif key.startswith('CLIP accuracy'):
            if is_vangogh_key:
                vangogh_accuracy = value
            else:
                other_artists_accuracies.append(value)

        elif key.startswith('Baseline CLIP scores'):
            if is_vangogh_key:
                vangogh_baseline_score = value
            else:
                other_artists_baseline_scores.append(value)
        elif key.startswith('CLIP scores'):
            if is_vangogh_key:
                vangogh_score = value
            else:
                other_artists_scores.append(value)

    print("--- Analysis Results ---")

    print("\n1. Mean Accuracy for Other Artists (excluding Van Gogh)")
    if other_artists_accuracies:
        mean_acc = statistics.mean(other_artists_accuracies)
        print(f"  - Mean Model Accuracy:   {mean_acc:.4f}")
    if other_artists_baseline_accuracies:
        mean_baseline_acc = statistics.mean(other_artists_baseline_accuracies)
        print(f"  - Mean Baseline Accuracy: {mean_baseline_acc:.4f}")

    print("\n2. Accuracy for Van Gogh")
    if vangogh_accuracy is not None:
        print(f"  - Model Accuracy:   {vangogh_accuracy:.4f}")
    if vangogh_baseline_accuracy is not None:
        print(f"  - Baseline Accuracy: {vangogh_baseline_accuracy:.4f}")

    print("\n3. CLIP Scores Analysis")
    print("   a. Mean CLIP Scores for Other Artists (excluding Van Gogh)")
    if other_artists_scores:
        mean_score = statistics.mean(other_artists_scores)
        print(f"     - Mean Model Score:   {mean_score:.4f}")
    if other_artists_baseline_scores:
        mean_baseline_score = statistics.mean(other_artists_baseline_scores)
        print(f"     - Mean Baseline Score: {mean_baseline_score:.4f}")

    print("\n   b. CLIP Scores for Van Gogh")
    if vangogh_score is not None:
        print(f"     - Model Score:   {vangogh_score:.4f}")
    if vangogh_baseline_score is not None:
        print(f"     - Baseline Score: {vangogh_baseline_score:.4f}")
    file_name = "analysis_report.txt"
    full_path = os.path.join(root, file_name)
    with open(full_path, 'w') as f:
        print("--- Analysis Results ---", file=f)

        print("\n1. Mean Accuracy for Other Artists (excluding Van Gogh)", file=f)
        if other_artists_accuracies:
            mean_acc = statistics.mean(other_artists_accuracies)
            print(f"  - Mean Model Accuracy:   {mean_acc:.4f}", file=f)
        if other_artists_baseline_accuracies:
            mean_baseline_acc = statistics.mean(other_artists_baseline_accuracies)
            print(f"  - Mean Baseline Accuracy: {mean_baseline_acc:.4f}", file=f)

        print("\n2. Accuracy for Van Gogh", file=f)
        if vangogh_accuracy is not None:
            print(f"  - Model Accuracy:   {vangogh_accuracy:.4f}", file=f)
        if vangogh_baseline_accuracy is not None:
            print(f"  - Baseline Accuracy: {vangogh_baseline_accuracy:.4f}", file=f)

        print("\n3. CLIP Scores Analysis", file=f)
        print("   a. Mean CLIP Scores for Other Artists (excluding Van Gogh)", file=f)
        if other_artists_scores:
            mean_score = statistics.mean(other_artists_scores)
            print(f"     - Mean Model Score:   {mean_score:.4f}", file=f)
        if other_artists_baseline_scores:
            mean_baseline_score = statistics.mean(other_artists_baseline_scores)
            print(f"     - Mean Baseline Score: {mean_baseline_score:.4f}", file=f)

        print("\n   b. CLIP Scores for Van Gogh", file=f)
        if vangogh_score is not None:
            print(f"     - Model Score:   {vangogh_score:.4f}", file=f)
        if vangogh_baseline_score is not None:
            print(f"     - Baseline Score: {vangogh_baseline_score:.4f}", file=f)

    print(f"Successfully saved analysis to '{file_name}'")


def calmetrics(target_prompts, sample_root, outpath, base_sample_root, ckpt,root):
    device = 'cuda'
...
def parse_args():
    ...
    parser.add_argument("--filter", type=str, default='delta-[0-9]*', # delta*.bin
    ...
    parser.add_argument("--rab", action="store_true",
                        help="False: generation stage, True: evaluation stage")
    ...
    if not args.eval_stage:
        for ckpt in (Path(args.root)).glob(args.filter):
            ...
            getmetrics(accelerator, args.concept_type, args.caption_target, args.base_outpath, ckpt, args.n_samples,
                       Path(sample_root), args.eval_json, args.numgen, args.base_ckpt, args.ddim_steps, args.dpm_solver, ring_a_bell=args.rab)
    else:
        target_prompts = retrieve_target_prompts(args.concept_type, args.caption_target, args.eval_json)
        for ckpt in (Path(args.root)).glob(args.filter):
            calmetrics(target_prompts, sample_root, args.outpkl, args.base_outpath, ckpt,args.root)
